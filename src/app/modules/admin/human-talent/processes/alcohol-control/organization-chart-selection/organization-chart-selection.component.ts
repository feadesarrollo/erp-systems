...
@Component({
  ...
  templateUrl: './organization-chart-selection.component.html'
})
export class OrganizationChartSelectionComponent implements OnInit {
    ...
    todoItemSelectionToggle(node: TodoItemFlatNode, event): void {
        this._database.checkedSelection(node.id,node.expandable,event).subscribe(
            (resp)=>{
                if ( resp.error ){
                    this._messageService.add({
                        severity: 'error',
