...
import { ScrollbarGeometry, ScrollbarPosition } from '@boby/directives/scrollbar/scrollbar.types';

/**
 * Wrapper directive for the Perfect Scrollbar: https://github.com/mdbootstrap/perfect-scrollbar
 */
@Directive({
    selector: '[bobyScrollbar]',
    exportAs: 'bobyScrollbar'
})
export class BobyScrollbarDirective implements OnChanges, OnInit, OnDestroy
{
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_bobyScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    @Input() bobyScrollbar: boolean = true;
    @Input() bobyScrollbarOptions: PerfectScrollbar.Options;
    ...
    {
        // Enabled
        if ( 'bobyScrollbar' in changes )
        {
            // Interpret empty string as 'true'
            this.bobyScrollbar = coerceBooleanProperty(changes.bobyScrollbar.currentValue);

            // If enabled, init the directive
            if ( this.bobyScrollbar )
            {
                this._init();
            }
        ...
        // Scrollbar options
        if ( 'bobyScrollbarOptions' in changes )
        {
            // Merge the options
            this._options = merge({}, this._options, changes.bobyScrollbarOptions.currentValue);
