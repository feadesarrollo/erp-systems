...
import { BobyMockApiService } from '@boby/lib/mock-api/mock-api.service';

@Injectable({
    providedIn: 'root'
})
export class BobyMockApiInterceptor implements HttpInterceptor
{
    /**
     * Constructor
     */
    constructor(
        @Inject(BOBY_MOCK_API_DEFAULT_DELAY) private _defaultDelay: number,
        private _bobyMockApiService: BobyMockApiService
    )
    {
    }
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>>
    {
        ...
        const {
                  handler,
                  urlParams
              } = this._bobyMockApiService.findHandler(request.method.toUpperCase(), request.url);
