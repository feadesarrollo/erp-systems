...
import { BobyNavigationItem, BobyVerticalNavigationAppearance, BobyVerticalNavigationMode, BobyVerticalNavigationPosition } from '@boby/components/navigation/navigation.types';
...
@Component({
    selector       : 'boby-vertical-navigation',
    templateUrl    : './vertical.component.html',
    styleUrls      : ['./vertical.component.scss'],
    animations     : bobyAnimations,
    encapsulation  : ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs       : 'bobyVerticalNavigation'
})
export class BobyVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: BobyVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: BobyVerticalNavigationMode = 'side';
    @Input() name: string = this._bobyUtilsService.randomId();
    @Input() navigation: BobyNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: BobyVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output() readonly appearanceChanged: EventEmitter<BobyVerticalNavigationAppearance> = new EventEmitter<BobyVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<BobyVerticalNavigationMode> = new EventEmitter<BobyVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> = new EventEmitter<boolean>();
    @Output() readonly positionChanged: EventEmitter<BobyVerticalNavigationPosition> = new EventEmitter<BobyVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<BobyNavigationItem> = new ReplaySubject<BobyNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<BobyNavigationItem> = new ReplaySubject<BobyNavigationItem>(1);
    ...
    private _bobyScrollbarDirectivesSubscription: Subscription;
    ...
    {
        return {
            'boby-vertical-navigation-animations-enabled'             : this._animationsEnabled,
            [`boby-vertical-navigation-appearance-${this.appearance}`]: true,
            'boby-vertical-navigation-hover'                          : this._hovered,
            'boby-vertical-navigation-inner'                          : this.inner,
            'boby-vertical-navigation-mode-over'                      : this.mode === 'over',
            'boby-vertical-navigation-mode-side'                      : this.mode === 'side',
            'boby-vertical-navigation-opened'                         : this.opened,
            'boby-vertical-navigation-position-left'                  : this.position === 'left',
            'boby-vertical-navigation-position-right'                 : this.position === 'right'
        };
    }
    ...
    /**
     * Setter for bobyScrollbarDirectives
     */
    @ViewChildren(BobyScrollbarDirective)
    set bobyScrollbarDirectives(bobyScrollbarDirectives: QueryList<BobyScrollbarDirective>)
    {
        // Store the directives
        this._bobyScrollbarDirectives = bobyScrollbarDirectives;

        // Return if there are no directives
        if ( bobyScrollbarDirectives.length === 0 )
        {
            return;
        }

        // Unsubscribe the previous subscriptions
        if ( this._bobyScrollbarDirectivesSubscription )
        {
            this._bobyScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._bobyScrollbarDirectivesSubscription =
            merge(
                ...
                .subscribe(() => {
                    ...
                    bobyScrollbarDirectives.forEach((bobyScrollbarDirective) => {
                        bobyScrollbarDirective.update();
                    });
                });
    }
    ...
    ngAfterViewInit(): void
    {
        setTimeout(() => {
            ...
            if ( !this._navigationContentEl.nativeElement.classList.contains('ps') )
            {
                ...
                const activeItem = this._navigationContentEl.nativeElement.querySelector('.boby-vertical-navigation-item-active');
            ...
            else
            {
                ...
                this._bobyScrollbarDirectives.forEach((bobyScrollbarDirective) => {

                    // Skip if not enabled
                    if ( !bobyScrollbarDirective.isEnabled() )
                    {
                        return;
                    }

                    // Scroll to the active element
                    bobyScrollbarDirective.scrollToElement('.boby-vertical-navigation-item-active', -120, true);
                });
            }
        });
    }
     ...
     * @param item
     */
    openAside(item: BobyNavigationItem): void
    {
     ...
     * @param item
     */
    toggleAside(item: BobyNavigationItem): void
    {
        // Toggle
    ...
    private _showOverlay(): void
    {
        ...
        this._overlay.classList.add('boby-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if ( this.transparentOverlay )
        {
            this._overlay.classList.add('boby-vertical-navigation-overlay-transparent');
        }
    ...
    private _showAsideOverlay(): void
    {
        ...
        this._asideOverlay.classList.add('boby-vertical-navigation-aside-overlay');
